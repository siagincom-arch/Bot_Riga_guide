...
### Текущий фокус
**Фаза:** M14 Меню + Категории + Рейтинг — **M14.1 (Меню) выполнено, ждет ревью UX**.
**Статус:**
- `docs/M14_PLAN.md` approved (все 6 вопросов §17 закрыты).
- `work/m14/next_session_claude.md` — план для следующей сессии Claude.
- `work/m14/ag-task-m14-1.md` — задача закрыта.
- **TTS аудио-ответы (M12) полностью запущены!** Исправлен критический баг `await write_to_file`.
- **Исправление багов RAG (Юрмальская черепаха) готово** (text_search.py адаптирован, пороги настроены).
**Следующий ход:**
1. Разрешение конфликтов и завершение мерджа ветки `m14-menu-categories-rating` в `master` (прямо сейчас!).
2. Деплой изменений на VPS (`89.167.11.154`) строго по `DEPLOY.md`.
3. Ревью UX в Telegram. После approve → Старт M14.2 (Транспорт + Лайфхаки) или M14.3 (Рейтинг).
