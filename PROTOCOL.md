...
### Текущий фокус
**Фаза:** M13 Динамическая База Знаний.
**Статус:** 
- **Фаза M13 реализована.** 
  - Реализована команда `/fact` для добавления пользовательских фактов.
  - Сделана мини-админка для подтверждения фактов (кнопки Approve / Reject) с занесением их в базу.
  - Реализован Web Search Loopback: факты, найденные через веб-поиск (Tavily), асинхронно сохраняются в Chroma как `Passage` (source="web_fact").
  - Обновлён системный промпт для генератора: пользовательские факты снабжаются обязательным дисклеймером для снятия исторической ответственности.
**Следующий ход:** Тестирование Voice + M13 модерации на VPS и финальный деплой аудио части.
