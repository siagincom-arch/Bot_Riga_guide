...
**Следующий ход:**
1. Natalja делает ревью UX в Telegram. После approve → мерж `m14-menu-categories-rating` в master + деплой на VPS.
2. Старт M14.2 (Транспорт + Лайфхаки) или M14.3 (Рейтинг).
...
**Задачи:**
- [x] openai.py: убран некорректный `await` перед методом `write_to_file` (метод является синхронным в SDK OpenAI). Это предотвращает падение асинхронного воркера при озвучке.
...
**Инсайты:** Метод `write_to_file` в SDK OpenAI 1.x является строго синхронным и блокирующим, в отличие от сетевых вызовов. Вызов `await` на нем возвращает `None` и роняет корутину с `TypeError`.
