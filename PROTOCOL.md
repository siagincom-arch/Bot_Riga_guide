...
### Текущий фокус
**Фаза:** M14 Меню + Категории + Рейтинг — **M14.1 (Меню) и исправления TTS/RAG успешно развернуты на VPS**.
**Статус:**
- `docs/M14_PLAN.md` approved.
- Все свежие исправления (TTS, RAG Юрмала, меню) объединены в `master` и задеплоены на VPS (`89.167.11.154`).
- Бот стабильно работает на сервере через Docker Compose (активен Long Polling).
- Все 188 автотестов успешно пройдены локально и в CI.
**Следующий ход:**
1. Ручной smoke-тест пользователем в Telegram (голосовые ответы, Юрмала, меню `/menu`).
2. Принятие решения по следующему приоритету: **M14.2 (Транспорт и Лайфхаки)** или **M14.3 (Система рейтингов)**.
3. Разработка выбранного подэтапа M14.

---

### 2026-05-21 | Запуск TTS аудио-ответов и исправление багов RAG (Юрмала) 🛠️ Antigravity
**Исполнитель:** Antigravity
**Задачи:**
- [x] openai.py: убран некорректный `await` перед методом `write_to_file` (метод является синхронным в SDK OpenAI) и успешно разрешен конфликт слияния. Логика генерации переписана с получением WAV-байтов напрямую в память через `response.read()` и надежным перекодированием в OGG/Opus через `ffmpeg` (совместимо со всеми мобильными клиентами Telegram iOS/Android).
- [x] Dockerfile: добавлена системная установка `ffmpeg` для надежной перекодировки аудио на VPS.
- [x] text_search.py: занижен порог acceptable дистанции до 0.30 и внедрен список триггеров не-рижских городов для ухода в dynamic fallback (Tavily). Юрмальская черепаха теперь корректно ищется в веб-поиске.
- [x] тесты: запуск тестов в контейнере `ingest` подтвердил полную работоспособность — все 188 тестов успешно пройдены (зелёные).

**Результат:** Аудио-ответы и RAG-эвристики полностью готовы к деплою на VPS.
**Источники:** Официальное SDK `openai`, документация `asyncio.subprocess`.
**Инсайты:** Метод `write_to_file` в SDK OpenAI 1.x является строго синхронным и блокирующим, в отличие от сетевых вызовов. Вызов `await` на нем возвращает `None` и роняет корутину с `TypeError`. Перекодирование WAV через ffmpeg в Ogg Opus с нужными метаданными и параметрами (`-c:a libopus -b:a 32k -ar 48000 -ac 1`) — единственный 100% надежный способ отправки голосовых в Telegram для воспроизведения на любых смартфонах.

---

### 2026-05-20 | Fix TTS: надёжная доставка аудио на сторонних мобильниках 🛠️ Antigravity
**Исполнитель:** Antigravity
**Задачи:**
- [x] `src/llm/openai.py` — заменён ненадёжный `stream_to_file` (который в разных версиях SDK sync/async) на явный `response.read()` + `Path.write_bytes()`. Добавлена проверка на пустой ответ (< 100 байт) и 1 автоматический retry с паузой 1.5 сек.
- [x] `src/bot/gateway.py` — TTS-воркер `_trigger_tts` теперь проверяет размер файла перед `send_voice` (пустые не отправляет), оборачивает отправку в `asyncio.wait_for(timeout=30)`, логирует chat_id и file_size.

**Причина бага:** `stream_to_file` в openai SDK v1.x+ может вернуть корутину, а может не вернуть — зависит от httpx backend. Предыдущий фикс с `inspect.iscoroutine` закрывал только часть случаев. Плюс при сетевых сбоях файл мог остаться 0 байт, и Telegram молча не доставлял его получателю.

**Результат:** 3 уровня защиты — retry на уровне OpenAI API, проверка размера файла, таймаут на отправку.

**Handoff:** нужен деплой на VPS для проверки на сторонних устройствах.
...
- [x] ui.py: `make_menu_keyboard()` (6 категорий 2×3) + `make_examples_keyboard()` (3 примера)
- [x] gateway.py: отправка двух клавиатур в `on_start` и `/menu`, callback-роутер `menu:*` с 6 заглушками
- [x] __main__.py: регистрация `on_menu`
- [x] test_ui.py: 6 новых тестов на структуру клавиатур (зелёные)

**Результат:** меню работает, smoke-тест в Telegram пройден.
**Решение по UX:** меню и примеры отправляются двумя отдельными клавиатурами, как изначально предполагалось в `M14_PLAN.md §4.2`.

**Handoff:** ветка `m14-menu-categories-rating`, коммит `8980c23`. Передаю Natalja на ревью UX в Telegram. После approve → мерж в master + деплой на VPS.
